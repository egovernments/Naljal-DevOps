terraform {
  backend "s3" {
    ...
    dynamodb_table = "assam-prod-s3-state"
...
data "aws_eks_cluster" "cluster" {
  depends_on = [module.eks_ondemand_managed_node_group]
  name = var.cluster_name
}
...
resource "aws_iam_role_policy" "karpenter_policy" {
  ...
  role   = module.eks_ondemand_managed_node_group.iam_role_name
  policy = jsonencode({
    "Version": "2012-10-17",
    "Statement": [
      {
...
module "karpenter" {
  ...
  node_iam_role_arn    = module.eks_ondemand_managed_node_group.iam_role_arn
